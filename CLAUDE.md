...
**Conversion reporting flow:** The `Conversion_Reporter` always registers regardless of credential status, so conversions are queued even during credential outages. `enqueue()` snapshots raw settings values (including `attribution_fraction` and `conversion_action_id`) into each payload. It reads click IDs in the structured format `['id' => string, 'captured_at' => int]` from core >= 1.9.0's `Click_ID_Store::get_for_hashes()`. Each job is deferred by `UPLOAD_DELAY_SECONDS` (21600 = 6 hours) from the click capture time via the core queue's `not_before` parameter, giving Google Ads time to register the click. Old clicks already past the 6h window get `not_before` in the past and are processed immediately. `process()` merges payload credentials with current settings as fallback — if the payload has empty credentials (queued during an outage), current settings fill the gaps. Exception: `conversion_action_id` always prefers the current settings value over the payload snapshot, because changing the action ID is intentional (e.g. replacing a misconfigured action) and old queued jobs must follow the new configuration. Derived values (resource name, attributed value) are computed at process time. If required credentials are still missing after merge, the job fails and will be retried. When settings are updated with valid credentials, `Plugin::on_settings_updated()` resets all failed Google Ads queue jobs to pending. The `Google_Ads_Client` handles OAuth2 token refresh and conversion upload via `wp_remote_post()`. Access tokens are cached in the `kntnt_ad_attr_gads_access_token` transient with a safety margin.
...
**Permanent failure detection:** `Google_Ads_Client::upload_click_conversion()` classifies partial failure errors as permanent or retryable via `is_permanent_partial_failure()`. Known permanent errors include CLICK_NOT_FOUND ("could not be attributed to a click"), CLICK_CONVERSION_ALREADY_EXISTS, CONVERSION_PRECEDES_EVENT, EXPIRED_EVENT, and INVALID_CONVERSION_ACTION_TYPE. The return array includes a `permanent_failure` boolean flag. `Conversion_Reporter::process()` returns `true` for permanent failures to stop the queue from retrying — the conversion is already recorded in the core plugin's stats regardless. Note: because `process()` returns `true`, the queue marks permanent failures as "Success" rather than "Failed". A future core plugin update could add support for a third status (e.g. `'abandoned'`) to show permanent failures accurately in the queue UI.

**Credential error notification:** `Conversion_Reporter::process()` checks the `credential_error` flag in the result array from `Google_Ads_Client` to decide whether to set the `kntnt_ad_attr_gads_credential_error` transient. The transient has no expiry — it persists until explicitly cleared. On successful upload the transient is deleted. Permanent API failures also clear the transient — a permanent failure (e.g. CLICK_NOT_FOUND) proves the API was reached and credentials are valid, so any stale credential error notice should be dismissed. `Settings_Page` hooks `admin_notices` to display a persistent error notice (visible on all admin pages, `manage_options` capability required) with a link to the settings page. `Plugin::on_settings_updated()` also clears the transient when settings are saved, so the notice disappears immediately after re-entering credentials. Only credential failures set the flag — other API errors (HTTP 4xx, partial failures) do not.
...
- `Conversion_Reporter::enqueue()` depends on the structured click ID format (`['id' => string, 'captured_at' => int]`) from core plugin >= 1.9.0. The `UPLOAD_DELAY_SECONDS` constant (21600 = 6 hours) determines how long after click capture the conversion upload is deferred.
