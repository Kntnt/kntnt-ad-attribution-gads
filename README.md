...
**I just created a conversion action and the log shows upload errors. Is something wrong?**

No. After creating a new conversion action, Google Ads requires approximately 6 hours before it accepts conversion uploads. You will see the error *"Can't import events to a conversion action that was just created. Try importing again in 6 hours."* in the diagnostic log during this period. The queue processor retries automatically, and all queued conversions will be uploaded once the waiting period has passed. No action is needed.
